...
Sean $\theta$ los parámetros de un modelo, $x$ la entrada, $y$ las salidas asociadas, y $J(\theta, x, y)$ la función de costo \cite{goodfellow2015explaining}. La función de costo se linealiza alrededor del valor actual de $\theta$. Sea $\epsilon \in \mathbb{R}^+$. Definamos la imagen adversaria 
...
\subsubsection{Carlini \& Wagner con norma $L_2$}
...
En la Figura \ref{CW} se observa que los ataques con norma $L_2$ y norma $L_\infty$ son prácticamente imperceptibles para nosotros, mientras que con el de norma $L_0$ sí se observan algunos pixeles modificados. Aunque este ataque puede ser dirigido, se usa de la forma no dirigida en este trabajo. En la Figura \ref{attack_examples} se muestra el tipo de ruido que generan los ataques FGSM, FGM y Carlini-Wagner, los cuales son cada vez más sutiles en ese respectivo orden. Con esos tres ataques se logra que la red clasifique erróneamente un 8 por un 3.
...
\begin{figure}[h!]
    \centering
    \includegraphics[width=0.43\textwidth]{images/CW/CW_norms.png}
    \caption{Los ataques diseñados por Carlini \& Wagner con cada una de las 3 normas que emplearon: $L^2$, $L_\infty$ y $L_0$, mostrados en ese orden en cada columna, sobre MNIST y sobre CIFAR-10. La imagen de cada renglón es la primera de su clase en el conjunto de evaluación. Dada la etiqueta correcta $l$, todas las clasificaciones (erróneas) en presencia de los ataques fueron $l+1\ (\text{mod}\ 10)$, i.e. fueron clasificadas con la etiqueta posterior a la correcta \cite{carlini2017evaluating}.}
    \label{CW}
\end{figure}
...
\begin{figure}[h!]
    ...
    \begin{subfigure}[b]{0.55\textwidth}
        ...
        \caption{FGSM con \texttt{epsilon = 0.066}}
        \label{fgsm_example}
    \end{subfigure}
    \begin{subfigure}[b]{0.55\textwidth}
        \centering
        \includegraphics[width=\textwidth]{images/fgm_example.png}
        \caption{FGM con \texttt{epsilon = 0.078}}
        \label{fgm_example}
    \end{subfigure}
    \begin{subfigure}[b]{0.55\textwidth}
        \centering
        \includegraphics[width=\textwidth]{images/CW_example.png}
        \caption{CW (no tiene \texttt{epsilon} como argumento)}
        \label{CW_example}
    \end{subfigure}
    \caption{Ruido de los ataques FGSM, FGM y Carlini-Wagner sobre MNIST, con los cuales un 8 es clasificado erróneamente por LeNet como un 3.}
